...
class CHelper extends Helper
{
	...
	public function __construct()
	{
		self::$LIM = sprintf(
			'%s %s %s',
			esc_html__('Lorem ipsum dolor sit amet, consectetur adipiscing elit, ', 'addons-for-elementor-builder'),
			esc_html__('sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ', 'addons-for-elementor-builder'),
			esc_html__('Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.', 'addons-for-elementor-builder')
		);
		$this->alignment = [
	...
	public function bse_txt(
		...
		$args = [];
		$args['label'] = $label;
		$args['type'] = $type == 'text' ? Controls_Manager::TEXT : ($type == 'text_area' ? Controls_Manager::TEXTAREA : Controls_Manager::WYSIWYG);
		if (!empty($default)) {
			$args['default'] = $default;
		}
		...
		if (strpos($id, '_dnmc') !== false) {
			$args['dynamic'] = ['active' => true];
		}
		if (strpos($id, '_dai') !== false) {
			$args['ai'] = ['active' => false];
		}
		$widget->add_control($id, $this->set_conditions($args, $conditions));
	}
	...
	public function mda(
		...
		$args = [];
		$args['label'] = !empty($label) ? $label : __('Image', 'addons-for-elementor-builder');
	...
	public function talmnt(
		...
		if ($responsive == 0) {
			$this->chse(
				...
				$id,
				$label,
				!is_rtl() ? $this->alignment : array_reverse($this->alignment),
				$selectors,
				1,
				$default,
				$conditions
	...
	public function clr(
		...
		$selectors = [];
		if (!empty($selector)) {
			$selectors[$selector] = 'color: {{VALUE}};fill: {{VALUE}}';
		}
		$this->clr_pckr(
			$widget,
			$id,
	...
	public function bg_clr(
		...
		$label = !empty($label) ? $label : __('Background Color', 'addons-for-elementor-builder');
		$selectors = [];
		if (!empty($selector)) {
