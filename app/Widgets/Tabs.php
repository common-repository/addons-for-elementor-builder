...
class Tabs extends Widget_Base
{
    ...
    public function register_controls()
    {
        $this->CHelper->add_cnt_sctn($this, 'cs1', esc_html__('Tabs', 'addons-for-elementor-builder'), function ($obj) {
            ...
            $this->CHelper->txt($items, 'tb_ttl', esc_html__('Title', 'addons-for-elementor-builder'), $txt, $txt, 1);
            $this->CHelper->hddn($items, 'tb_cnt_typ');
            $this->CHelper->wysiwyg($items, 'tb_cnt', esc_html__('Content', 'addons-for-elementor-builder'), CHelper::$LIM, '', 0, ['tb_cnt_typ' => '']);
        ...
        $this->CHelper->add_cnt_sctn($this, 'cs2', esc_html__('Tab Items', 'addons-for-elementor-builder'), function ($obj) {
            ...
            $this->CHelper->res_talmnt($obj, 'tb_itm_almnt', $slctr);
    ...
    protected function render_attrs($settings = [])
    {
        ...
        $classes[] = $settings['tb_itms_almnt'] ? 'afeb-tabs-' . $settings['tb_itms_almnt']  : '';
        ...
        echo wp_kses_post($this->get_render_attribute_string('tabs'));
    }
    ...
    protected function render()
    {
        ...
        foreach (array_reverse($settings['tbil']) as $i => $item) {
            ...
            $content = Helper::hpv($item['tb_cnt']) ?  $item['tb_cnt'] :
                wp_kses(Helper::front_notice(sprintf(STRS::msgs('use_pro'), Helper::pro_badge()), 'error'), Helper::allowed_tags(['div']));
